...
export const instructorController = {
    ...
    updateProfile: async (req, res) => {
        try {
            ...
            const affected = await InstructorModel.update(instructor.instructor_id, { department, office });
            if (!affected) return res.status(400).json({ message: "No changes made", success: false });
            const updated = await InstructorModel.readById(instructor.instructor_id);
    ...
    getCourses: async (req, res) => {
        try {
            const instructor = await InstructorModel.readByUserId(req.user.user_id);
            if (!instructor) return res.status(404).json({ message: "Instructor not found", success: false });
            const courses = await InstructorModel.readCourses(instructor.instructor_id);
    ...
    getExams: async (req, res) => {
        try {
            const instructor = await InstructorModel.readByUserId(req.user.user_id);
            if (!instructor) return res.status(404).json({ message: "Instructor not found", success: false });
            const exams = await ExamModel.readByInstructorId(instructor.instructor_id);
    ...
    createExam: async (req, res) => {
        try {
            ...
            const newExam = await ExamModel.readById(examId);
    ...
    updateExam: async (req, res) => {
        try {
            const { id } = req.params;
            const exam = await ExamModel.readById(id);
            ...
            const updated = await ExamModel.update(id, {
                title,
                description,
                duration_minutes,
                total_marks,
            ...
            const updatedExam = await ExamModel.readById(id);
    ...
    deleteExam: async (req, res) => {
        try {
            ...
            await QuestionModel.deleteByExamId(id);
    ...
    getQuestions: async (req, res) => {
        try {
            ...
            const questions = await QuestionModel.readByExamId(examId);
            return res.status(200).json({
    ...
    createQuestion: async (req, res) => {
        try {
            ...
            const newQuestion = await QuestionModel.readById(questionId);
    ...
    updateQuestion: async (req, res) => {
        try {
            const { id } = req.params;
            const question = await QuestionModel.readById(id);

            if (!question) return res.status(404).json({ message: "Question not found", success: false });

            const exam = await ExamModel.readById(question.exam_id);
            const instructor = await InstructorModel.readByUserId(req.user.user_id);
            ...
            const updated = await QuestionModel.update(id, {
                question_text,
                question_type,
                options: options ? JSON.stringify(options) : null,
                correct_answer,
            ...
            const updatedQuestion = await QuestionModel.readById(id);
            return res.status(200).json({
    ...
    getExamStudents: async (req, res) => {
        try {
            ...
            const students = await ExamModel.getEnrolledStudents(examId);
            return res.status(200).json({
